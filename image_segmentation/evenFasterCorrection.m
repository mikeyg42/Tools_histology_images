function outputRGBim = evenFasterCorrection(RGB)
se = strel('disk', 12, 8);

RGBpad = padarray(RGB, [3, 3], 'replicate', 'both');
    
smallPADsz = size(RGBpad, 1:3);
onesMat = repmat(1, smallPADsz(1), smallPADsz(2)*3); % twice as fast as ones([m,n])
...
ncol = smallPADsz(2);
RGBadj_pad = zeros(smallPADsz, 'like',RGB);
