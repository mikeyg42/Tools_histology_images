...
listMethods = {'DONE',...
  ...
  %'Texture segmentation Gabor+LBP, then FCM',...
...
switch choice
    ...
    case 'Full color segmentation'
        imgC = imresize(imgCurrent, 0.5, {@oscResampling, 4});
        [BWmask, ~] = fullColorSegmentation(imgC);
        outOfBoundsMask = imresize(BWmask, size(imgCurrent, 1:2), {@oscResampling, 4});
    
    case 'Partial + fractional derivatives segmentation'
        outOfBoundsMask = vectorCalcSegment(imgCurrent);
    ...
    case 'Texture segmentation options GUI (no Gabor)' 
        ...
        outOfBoundsMask = textureFilterGUI(imgRGB); 
    ...
    case 'Color segment with geodesics, w/ provided foreground sample'
        ...
        assert(islogical(outOfBoundsMask));
        outOfBoundsMask = cleanMask(outOfBoundsMask);
...
function guiCloseFuntion(src,evt)
